...
type LambdaEvent struct {
	...
	EventImage         InputImage   `json:"event_image"`
...
func Start(ctx context.Context, lambdaEvent LambdaEvent) (response, error) {
	...
	switch {
	// if event image is set, only sync this image
	case lambdaEvent.EventImage.ImageName != "":
		images = []InputImage{lambdaEvent.EventImage}
	...
	for _, i := range images {
		...
		tagsToSync, err := svc.getTagsTosync(&i, ecrImageName, lambdaEvent.EcrRepoPrefix, lambdaEvent.MaxResults, lambdaEvent.CheckDigest, environmentVars)
		if err != nil {
			return returnErr(err, environmentVars.slackOAuthToken, lambdaEvent.SlackChannelID, errSubject,
